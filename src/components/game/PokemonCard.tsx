    ...
    return (
      ...
      >
        <div className="flex items-center justify-between gap-1">
          ...
          <span className="text-xs font-mono text-gray-400">{pokemon.currentHp}/{pokemon.hp}</span>
        </div>
        {/* HP bar */}
        <div className="w-full h-1 bg-gray-800 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all ${hpColor}`} style={{ width: `${Math.max(0, hpPercent)}%` }} />
        </div>
        {/* Energy pips */}
        ...
        {pokemon.specialConditions.length > 0 && (
          <div className="flex flex-wrap gap-0.5">
            {pokemon.specialConditions.map(c => (
              <span key={c} className={`text-[9px] px-1 rounded border ${conditionColors[c]}`}>{c.slice(0,3)}</span>
            ))}
          </div>
        )}
        {/* All attacks small */}
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isOpponent ? 'border-red-800/60 bg-gradient-to-b from-red-950/30 to-black' : 'border-emerald-800/60 bg-gradient-to-b from-emerald-950/30 to-black',
          onClick && 'cursor-pointer hover:border-white/50',
      ...
      >
        ...
        <div className="flex items-start justify-between gap-2">
          <div>
            <div className="flex items-center gap-2">
              ...
              {pokemon.isEX && <span className="text-xs bg-yellow-900/60 text-yellow-300 border border-yellow-700 px-1.5 rounded">EX</span>}
            </div>
          ...
          <div className="text-right">
            <div className="text-2xl font-mono font-bold text-white">{pokemon.currentHp}</div>
          ...
          {pokemon.attacks.map((attack, i) => (
            <div key={i} className="flex items-start justify-between gap-2">
              <div className="flex-1">
                ...
                {attack.text && <div className="text-[11px] text-gray-400 mt-0.5">{attack.text}</div>}
              </div>
              <span className="text-lg font-mono font-bold text-white shrink-0">{attack.damage || '—'}</span>
            </div>
          ))}
        </div>
        ...
        {pokemon.ability && (
          <div className="border border-purple-900/50 rounded-lg p-2 bg-purple-950/20">
            <div className="text-[10px] text-purple-400 uppercase tracking-wider mb-0.5">Ability</div>
        ...
        <div className="flex items-center gap-3 text-xs text-gray-500 font-mono border-t border-gray-800 pt-2">
          {pokemon.weakness && <span>W: {pokemon.weakness.type} {pokemon.weakness.multiplier}</span>}
          {pokemon.resistance && <span>R: {pokemon.resistance.type} {pokemon.resistance.modifier}</span>}
          <span>Retreat: {pokemon.retreatCost}</span>
          {pokemon.damage > 0 && <span className="text-red-400 ml-auto">{pokemon.damage} dmg</span>}
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-1">
        ...
        <span className="text-base font-mono font-bold text-white shrink-0">{pokemon.currentHp}<span className="text-xs text-gray-500">/{pokemon.hp}</span></span>
      </div>
      ...
      {pokemon.specialConditions.length > 0 && (
        <div className="flex flex-wrap gap-0.5">
          {pokemon.specialConditions.map(c => (
            <span key={c} className={`text-[9px] px-1 py-0.5 rounded border ${conditionColors[c]}`}>{c}</span>
          ))}
        </div>
      )}
...
export function EmptySlot({ label, isOpponent }: { label: string; isOpponent?: boolean }) {
  return (
    <div className={`rounded-lg border border-dashed p-3 flex items-center justify-center min-h-[80px]
